...
$artifactDirectory = Join-Path $projectRoot 'test-artifacts\V003-C010.1'
$standaloneRelativePath = 'test-artifacts/V003-C010.1/standalone-js-300-final-card.html'

Push-Location $projectRoot
try {
    $branch = (& git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne $expectedBranch) { throw "A V003-C010.1 kapu csak a develop/V003 agon futhat. Aktualis: $branch" }
    New-Item -ItemType Directory -Force -Path $artifactDirectory | Out-Null

    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\tools\validate-v003-c010.ps1'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & node '.\tools\audit-v003-c009-api-links.mjs' '--output=test-artifacts/V003-C010.1/exact-api-link-audit.json'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    ...
    Write-Output 'V003_C0101_VALIDATION_PASS'
    Write-Output "BRANCH=$branch"
    Write-Output 'MINING_COMPACT_TIE_LABEL=PASS'
    Write-Output 'REFINERY_COMPACT_TIE_LABEL=PASS'
    Write-Output 'LOWER_RANKED_REFINERY_EXCLUDED=PASS'
    Write-Output 'MAIN_STANDALONE_SHARED_PROJECTION=PASS'
