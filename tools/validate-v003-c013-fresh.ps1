...
$sourceHead = '6abae928b7d2f81e0b5eee2976a652feb0577c8d'
$artifactDirectory = Join-Path $projectRoot 'test-artifacts\V003-C013\fresh-release-candidate'
...
$temporaryCandidate = Join-Path $env:TEMP ('spg-v003-c013-fresh-' + [guid]::NewGuid().ToString('N') + '.html')
...
$lines.Add('V003-C013 fresh release candidate validation')
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'A D2B source HEAD nem ose az aktualis HEAD-nek.' }
    & git diff --quiet $sourceHead -- 'sPg Crafting List.html'
    if ($LASTEXITCODE -ne 0) { throw 'Az application HTML elter a D2B source HEAD-tol; C013 STOP.' }
    if (-not (Test-Path -LiteralPath '.\test-artifacts\V003-C013\INVALIDATED_BY_C012.5_RELEASE_BLOCKER.md')) { throw 'A regi C013 invalidacios marker hianyzik.' }

    Invoke-BoundedCheck 'static-single-file' 'powershell.exe' @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', '.\tools\validate-baseline.ps1')
    Invoke-BoundedCheck 'candidate-build' 'node' @('.\tools\build-v003-c013-fresh-candidate.mjs')
    Invoke-BoundedCheck 'candidate-rebuild' 'node' @('.\tools\build-v003-c013-fresh-candidate.mjs', "--output=$temporaryCandidate")
    ...
    if ($candidateShaBefore -ne $temporarySha -or $candidateBytes -ne (Get-Item -LiteralPath $temporaryCandidate).Length) { throw 'A fresh candidate determinisztikus rebuildje elter.' }
    $manifest = Get-Content -Raw -LiteralPath $manifestPath | ConvertFrom-Json
    if ($manifest.sourceHead -ne $sourceHead -or $manifest.sha256 -ne $candidateShaBefore -or [int64]$manifest.bytes -ne $candidateBytes) { throw 'A fresh candidate manifest elter.' }
    ...
    Invoke-BoundedCheck 'fresh-candidate-fr86-target' 'node' @('.\tools\run-v003-c013-fresh-tests.mjs')

    $candidateShaAfter = (Get-FileHash -Algorithm SHA256 -LiteralPath $candidatePath).Hash.ToLowerInvariant()
    if ($candidateShaAfter -ne $candidateShaBefore) { throw 'A candidate SHA megvaltozott a regresszio alatt.' }
    & git diff --quiet $sourceHead -- 'sPg Crafting List.html'
    if ($LASTEXITCODE -ne 0) { throw 'Az application code megvaltozott a C013 alatt.' }
    ...
    $evidence = [ordered]@{
        ...
        candidate = [ordered]@{ path = 'test-artifacts/V003-C013/fresh-release-candidate/sPg Crafting List V003 RC.html'; bytes = $candidateBytes; sha256Before = $candidateShaBefore; sha256After = $candidateShaAfter; immutable = $true; singleFile = $true; localRuntimeSidecars = 0 }
        regression = [ordered]@{ C001_C0124_M1_M61_C04 = 'PASS'; C0125A_C0125C3B2 = 'PASS'; C0125D1IntegratedFr86 = 'PASS'; staticSingleFile = 'PASS' }
        fr86 = $targetEvidence.integratedFr86
    ...
    [System.IO.File]::WriteAllLines($logPath, $lines, [System.Text.UTF8Encoding]::new($false))
    Write-Output "V003_C013_FRESH_AUTOMATED_PASS candidate=$candidatePath bytes=$candidateBytes sha256=$candidateShaAfter"
} finally {
