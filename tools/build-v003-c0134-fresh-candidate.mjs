...
const defaultOutput = path.join(projectDirectory, "test-artifacts", "V003-C013.4", "fresh-release-candidate", "sPg Crafting List V003 RC.html");
...
assert.equal(git(["branch", "--show-current"]).toString("utf8").trim(), "develop/V003", "A C013.4 builder csak a develop/V003 ágon futhat.");
assert.equal(git(["rev-parse", "HEAD"]).toString("utf8").trim(), sourceHead, "A C013.4 candidate kizárólag az exact C013.3 checkpointból épülhet.");
...
assert.deepEqual(workingBytes, sourceBytes, "A working application nem byte-azonos a C013.3 source HEAD-del.");
...
console.log("V003_C0134_FRESH_CANDIDATE_BUILD_PASS");
