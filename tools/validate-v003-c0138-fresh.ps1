...
$sourceHead = '490ed6fc3e94f2361c7448650a08752fa5f3c8c7'
$artifactDirectory = Join-Path $projectRoot 'test-artifacts\V003-C013.8\fresh-release-candidate'
...
$temporaryCandidate = Join-Path $env:TEMP ('spg-v003-c0138-fresh-' + [guid]::NewGuid().ToString('N') + '.html')
$expectedBlockedC0136Sha = '4489a48ff50e6652b89684dce522e35203557e33753d73b1c98aa5588193a7ed'
...
$lines.Add('V003-C013.8 fresh release candidate validation')
...
try {
    ...
    if ($head -ne $sourceHead) { throw "A C013.8 exact source HEAD elter: $head" }
    & git diff --quiet $sourceHead -- 'sPg Crafting List.html'
    if ($LASTEXITCODE -ne 0) { throw 'Az application HTML elter a C013.7 source HEAD-tol; C013.8 STOP.' }
    $appShaBefore = (Get-FileHash -Algorithm SHA256 -LiteralPath '.\sPg Crafting List.html').Hash.ToLowerInvariant()

    $blockedC0136Path = '.\test-artifacts\V003-C013.6\fresh-release-candidate\sPg Crafting List V003 RC.html'
    $blockedC0136Sha = (Get-FileHash -Algorithm SHA256 -LiteralPath $blockedC0136Path).Hash.ToLowerInvariant()
    if ($blockedC0136Sha -ne $expectedBlockedC0136Sha) { throw 'A blokkolt C013.6 candidate megvaltozott.' }

    Invoke-BoundedCheck 'static-single-file' 'powershell.exe' @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', '.\tools\validate-baseline.ps1')
    Invoke-BoundedCheck 'candidate-build' 'node' @('.\tools\build-v003-c0138-fresh-candidate.mjs')
    Invoke-BoundedCheck 'candidate-rebuild' 'node' @('.\tools\build-v003-c0138-fresh-candidate.mjs', "--output=$temporaryCandidate")
    ...
    if ($candidateShaBefore -ne $temporarySha -or $candidateBytes -ne (Get-Item -LiteralPath $temporaryCandidate).Length) { throw 'A C013.8 candidate determinisztikus rebuildje elter.' }
    $manifest = Get-Content -Raw -LiteralPath $manifestPath | ConvertFrom-Json
    if ($manifest.sourceHead -ne $sourceHead -or $manifest.sha256 -ne $candidateShaBefore -or [int64]$manifest.bytes -ne $candidateBytes) { throw 'A C013.8 manifest elter.' }
    ...
    Invoke-BoundedCheck 'candidate-d1-c0131-c0133-c0135-c0137-target' 'node' @('.\tools\run-v003-c0138-fresh-tests.mjs')
    ...
    if ([int]$liveAudit.pickerSafety.visibleDuplicatePickerCount -ne 0) { throw 'Lathato duplicate picker material maradt.' }
    if ([int]$liveAudit.pickerSafety.guessedCanonicalUuidCount -ne 0) { throw 'Bizonyitek nelkuli canonical UUID projekcio jelent meg.' }

    $targetEvidence = Get-Content -Raw -LiteralPath (Join-Path $artifactDirectory 'target-evidence.json') | ConvertFrom-Json
    $c0137Evidence = $targetEvidence.userDataIndependentCanonicalPicker
    if (@($c0137Evidence.invariance).Count -ne 4 -or @($c0137Evidence.invariance | Where-Object { -not $_.equal }).Count -ne 0) { throw 'A negy C013.7 canonical invariance fixture nem PASS.' }
    $titanium = @($c0137Evidence.invariance | Where-Object { $_.name -eq 'Titanium' })[0]
    if ($titanium.pickerCanonicalUuidNoUserData -ne '64978449-1d87-4a16-ba55-4b5f94fee217' -or $titanium.pickerCanonicalUuidWithLegacyUserData -ne '64978449-1d87-4a16-ba55-4b5f94fee217') { throw 'A Titanium picker User Data-fuggetlensege elter.' }
    if ($c0137Evidence.titanium.logicalMaterialCount -ne 1 -or $c0137Evidence.titanium.batchCount -ne 3) { throw 'A Titanium 1 logical material / 3 batch invariant elter.' }

    $candidateShaAfter = (Get-FileHash -Algorithm SHA256 -LiteralPath $candidatePath).Hash.ToLowerInvariant()
    if ($candidateShaAfter -ne $candidateShaBefore) { throw 'A C013.8 candidate SHA megvaltozott a regresszio alatt.' }
    $appShaAfter = (Get-FileHash -Algorithm SHA256 -LiteralPath '.\sPg Crafting List.html').Hash.ToLowerInvariant()
    if ($appShaAfter -ne $appShaBefore) { throw 'Az application code megvaltozott a C013.8 alatt.' }
    & git diff --quiet $sourceHead -- 'sPg Crafting List.html'
    ...
    $previousAuditPath = '.\test-artifacts\V003-C013.6\fresh-release-candidate\active-4.10-audit\active-4.10-material-identity-audit.json'
    $previousAudit = Get-Content -Raw -LiteralPath $previousAuditPath | ConvertFrom-Json
    $auditChanged = [bool]($liveAudit.picker.userFacingMaterialNameCount -ne $previousAudit.picker.userFacingMaterialNameCount -or $liveAudit.picker.visiblePickerOptionCount -ne $previousAudit.picker.visiblePickerOptionCount -or $liveAudit.picker.exactCanonicalMultiUuidMaterialCount -ne $previousAudit.picker.exactCanonicalMultiUuidMaterialCount -or $liveAudit.picker.unresolvedDuplicateCount -ne $previousAudit.picker.unresolvedDuplicateCount)
    $evidence = [ordered]@{
        cycle = 'V003-C013.8'
        status = 'AUTOMATED_PASS_CHROME_PENDING'
        sourceHead = $sourceHead
        candidate = [ordered]@{ path = 'test-artifacts/V003-C013.8/fresh-release-candidate/sPg Crafting List V003 RC.html'; bytes = $candidateBytes; sha256Before = $candidateShaBefore; sha256After = $candidateShaAfter; immutable = $true; singleFile = $true; embeddedCss = $true; embeddedJavaScript = $true; localRuntimeSidecars = 0 }
        regression = [ordered]@{ C001_C0122_M1_M61_C04 = 'PASS'; C0123 = 'PASS'; C0124 = 'PASS'; C0125A_C0125C3B2 = 'PASS'; C0125D1 = 'PASS'; C0131 = 'PASS'; C0133 = 'PASS'; C0135 = 'PASS'; C0137 = 'PASS'; staticSingleFile = 'PASS'; backupRestore = 'PASS' }
        canonicalPicker = [ordered]@{ invariance = @($c0137Evidence.invariance); titaniumLogicalMaterials = $c0137Evidence.titanium.logicalMaterialCount; titaniumBatches = $c0137Evidence.titanium.batchCount; legacyBatchPreserved = $true; sourceProvenance = 'PASS'; destructiveMigration = $false; insertionOrderIndependent = $true }
        activeMaterialAudit = [ordered]@{ activeScVersion = $liveAudit.activeScVersion; fetched = $liveAudit.fetched; picker = $liveAudit.picker; pickerSafety = $liveAudit.pickerSafety; changedFromC0136AuditCounts = $auditChanged }
        ...
        blockedC0136Candidate = [ordered]@{ status = 'UNCHANGED_BLOCKED_INVALIDATED'; sha256 = $blockedC0136Sha }
    ...
    $lines.Add('C0137_CANONICAL_INVARIANCE=PASS')
    $lines.Add('VISIBLE_PICKER_DUPLICATES=0')
    $lines.Add('GUESSED_CANONICAL_UUID=0')
    $lines.Add('BLOCKED_C0136_CANDIDATE_UNCHANGED=PASS')
    $lines.Add('V001_V002_INTEGRITY=PASS')
    $lines.Add('APPLICATION_CODE_CHANGED=NO')
    $lines.Add('V003_TAG_RELEASE=NONE')
    [System.IO.File]::WriteAllLines($logPath, $lines, [System.Text.UTF8Encoding]::new($false))
    Write-Output "V003_C0138_FRESH_AUTOMATED_PASS candidate=$candidatePath bytes=$candidateBytes sha256=$candidateShaAfter"
} finally {
