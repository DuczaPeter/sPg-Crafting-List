#Requires -Version 5.1
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

$projectRoot = Split-Path -Parent $PSScriptRoot
$expectedBranch = 'develop/V003'
$c0123Baseline = '9c789bada14fce50d58f5f762ce693935ddd331f'
$expectedV002Commit = 'b326aaff5838aafd5b1f13b16982c29a0e150e35'
$expectedV002Sha256 = 'de2d59b4203862167d90f8aa598ec6b043ea0556ead1afe7e067f69d659f2357'
$v002Artifact = Join-Path $projectRoot 'releases\V002\sPg Crafting List.html'
$invalidatedManifest = Join-Path $projectRoot 'test-artifacts\V003-C013\release-candidate\candidate-manifest.json'

Push-Location $projectRoot
try {
    $branch = (& git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne $expectedBranch) { throw "A V003-C012.4 kapu csak a develop/V003 agon futhat. Aktualis: $branch" }
    & git merge-base --is-ancestor $c0123Baseline HEAD
    if ($LASTEXITCODE -ne 0) { throw "A C012.3 baseline nem ose az aktualis HEAD-nek: $c0123Baseline" }

    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\tools\validate-v003-c0123.ps1'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & node '.\tools\run-v003-c0124-tests.mjs'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    $manifest = Get-Content -Raw -LiteralPath $invalidatedManifest | ConvertFrom-Json
    if ($manifest.status -ne 'INVALIDATED_BY_C012.3_RELEASE_BLOCKER') { throw "A C013 candidate statusza megvaltozott: $($manifest.status)" }
    $v002Commit = (& git rev-parse 'V002^{commit}').Trim()
    if ($LASTEXITCODE -ne 0 -or $v002Commit -ne $expectedV002Commit) { throw "A V002 tag dereferalt commitja elter: $v002Commit" }
    $v002Sha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $v002Artifact).Hash.ToLowerInvariant()
    if ($v002Sha256 -ne $expectedV002Sha256) { throw "A stabil V002 HTML SHA-256 erteke elter: $v002Sha256" }
    if (@(& git tag --list 'V003*').Count -gt 0) { throw 'A C012.4 alatt V003 tag jelent meg.' }
    if (Test-Path -LiteralPath (Join-Path $projectRoot 'releases\V003')) { throw 'A C012.4 alatt releases/V003 mappa jelent meg.' }

    Write-Output 'V003_C0124_VALIDATION_PASS'
    Write-Output "BRANCH=$branch"
    Write-Output 'NUMERIC_DRAFT_COMMIT_EDITOR=PASS'
    Write-Output 'CRAFT_1_TO_11452=PASS'
    Write-Output 'CRAFT_11452_TO_3=PASS'
    Write-Output 'TARGET_Q_800_TO_950=PASS'
    Write-Output 'TEMPORARY_EMPTY_AND_CARET_STABILITY=PASS'
    Write-Output 'ENTER_BLUR_CHANGE_TAB=PASS'
    Write-Output 'FULL_C001_C0123_M1_M61_C04_REGRESSION=PASS'
    Write-Output 'C013_CANDIDATE=INVALIDATED_BY_C012.3_RELEASE_BLOCKER'
    Write-Output "V002_TAG_COMMIT=$v002Commit"
    Write-Output "V002_HTML_SHA256=$v002Sha256"
} finally {
    Pop-Location
}
