...
$artifactDirectory = Join-Path $projectRoot 'test-artifacts\V003-C010'
$standaloneRelativePath = 'test-artifacts/V003-C010/standalone-js-300-final-card.html'

Push-Location $projectRoot
try {
    $branch = (& git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne $expectedBranch) { throw "A V003-C010 kapu csak a develop/V003 agon futhat. Aktualis: $branch" }
    New-Item -ItemType Directory -Force -Path $artifactDirectory | Out-Null

    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File '.\tools\validate-v003-c0081.ps1'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & node '.\tools\audit-v003-c009-api-links.mjs' '--output=test-artifacts/V003-C010/exact-api-link-audit.json'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    ...
    Write-Output 'V003_C010_VALIDATION_PASS'
    Write-Output "BRANCH=$branch"
    Write-Output 'DEFAULT_VIEW=BLUEPRINT_BROWSER_PLUS_FINAL_CARD_ONLY'
    Write-Output 'CRAFTING_LIST_DASHBOARD=SEPARATE_NAVIGATION_ONLY'
