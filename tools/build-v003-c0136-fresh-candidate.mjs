...
const defaultOutput = path.join(projectDirectory, "test-artifacts", "V003-C013.6", "fresh-release-candidate", "sPg Crafting List V003 RC.html");
...
assert.equal(git(["branch", "--show-current"]).toString("utf8").trim(), "develop/V003", "A C013.6 builder csak a develop/V003 agon futhat.");
assert.equal(git(["rev-parse", "HEAD"]).toString("utf8").trim(), sourceHead, "A C013.6 candidate kizarolag az exact C013.5 checkpointbol epulhet.");
...
assert.deepEqual(workingBytes, sourceBytes, "A working application nem byte-azonos a C013.5 source HEAD-del.");
...
console.log("V003_C0136_FRESH_CANDIDATE_BUILD_PASS");
