...
$artifactDirectory = Join-Path $projectRoot 'test-artifacts\V003-C004'

Push-Location $projectRoot
try {
    $branch = (& git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne $expectedBranch) {
        throw "A V003-C004 kapu csak a develop/V003 agon futhat. Aktualis: $branch"
    }
    ...
    & node '.\tools\run-m6-tests.mjs' '--artifact=test-artifacts/V003-C004/standalone-js-300-top3.html'
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    $radarSha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $radarSource).Hash.ToLowerInvariant()
    if ($radarSha256 -ne $expectedRadarSha256) {
        throw "A Radar Signature forraskep SHA-256 erteke elter: $radarSha256"
    }
    ...
    if ($v003Tags.Count -gt 0) {
        throw "A fejlesztesi ciklus alatt V003 tag jelent meg: $($v003Tags -join ', ')"
    }
    if (-not (Test-Path -LiteralPath (Join-Path $artifactDirectory 'radar-signature-audit.json'))) {
        throw 'A Radar Signature audit artifact hianyzik.'
    }
    if (-not (Test-Path -LiteralPath (Join-Path $artifactDirectory 'scmdb-comparison-audit.json'))) {
        throw 'Az SCMDB osszehasonlito audit artifact hianyzik.'
    }

    Write-Output 'V003_C004_VALIDATION_PASS'
